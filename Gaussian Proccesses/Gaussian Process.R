...
calcSigma <- function(X1,X2,l=1) {
  ...
  for (i in 1:nrow(Sigma)) {
    for (j in 1:ncol(Sigma)) {
       Sigma[i,j] <- exp(-0.5*(abs(X1[i]-X2[j])/l)^2)# Uses the squared exponential kernel
    }
  }
  return(Sigma)
}
...
values <- gather(values, key = var, value = value, -x)
...
fig2b <- ggplot(values,aes(x=x,y=value)) +
  geom_line(aes(group=var), colour="grey80")+
  geom_line(data=data.frame(x.star = x.star, f.star.bar = f.star.bar),aes(x=x.star,y=f.star.bar),colour="red", size=1) + 
...
gg <- ggplot(values, aes(x=x,y=value)) + 
  geom_line(aes(group=var), colour="grey80") +
  geom_line(data = data.frame(x.star = x.star, f.bar.star = f.bar.star), aes(x=x.star,y=f.bar.star),colour="red", size=1) + 
